...
#define ProgName           "CB"
#define VersionNumber      "Version 0.63"
#define LastUpdated        "05.12.2005" /* MT */
...
/*      Changelog:                                                   */
/*                                                                   */
/* MT   0.63  Fixed a bug in LocalRepartitioningGeneral              */
/* MT   0.62  CreateNewCodebook modified to copy GenerationMethod    */
/* MT   0.61  Fixed error in TrainingSetID (TRAININGOB -> TRAINING)  */
/* VH   0.60  Added kNN Graph routines (API and file IO)             */
...
#include "../../include/ikmeans/owntypes.h"
...
VECTORTYPE  CreateEmptyVector(int Vsize)
{
  ...
  v = (VECTORTYPE) allocateb( Vsize * sizeof(VECTORELEMENT));
  if( v == NULL )
    {
...
void AllocateMemoryForBook(CODEBOOK* CB)
{
  int i;

  CB->Book = (BOOKTYPE) allocateb(BookSize(CB) * sizeof(BOOKNODE));

  if( CB->Book == NULL )
    {
    ErrorMessage("ERROR: allocateb CB->Book\n");
    ExitProcessing(-1);
    }
...
void CopyNode(BOOKNODE* source, BOOKNODE* dest, int Vsize)
{
   ...
   deallocateb(dest->name);

   if ( source->name )
     {
     dest->name = (char*) allocateb( (strlen(source->name)+1) * sizeof(char) );
...
void ChangeVectorName( BOOKNODE* node, char* NewName)
{
  ...
  if ( NewName == NULL)
    ...
    {
    node->name = (char*) allocateb((strlen(NewName)+1) * sizeof(char));
...
void CodebookCentroid(CODEBOOK* CB, VECTORTYPE v)
{
  ...
  SumCounter = (llong *) allocateb( VectorSize(CB) * sizeof(llong) );
...
static int ReadOldCodebookFileHeader(FILE* f, CODEBOOK* CB)
{
  int ret = fscanf(f, "%i\n", &(CB->BlockSizeX));
  ret = fscanf(f, "%i\n", &(CB->BlockSizeY));
  ret = fscanf(f, "%i\n", &(CB->CodebookSize));
  ...
  if( strncmp(CodebookID, CB->Versionstr, strlen(CodebookID)) == 0 )
    {
    if (fgets(CB->GenerationMethod, MaxGenMethodLength, f) != NULL)
...
static int ReadNewCodebookFileHeader(FILE* f, CODEBOOK* CB)
{
  char tempchar;
...
static int ReadCodebookFileHeader(FILE* f, CODEBOOK* CB)
{
  ...
  if (fgets(CB->Versionstr, MaxVersionLength, f) != NULL)
...
static void ReadVectorNameFromFile(FILE* f, char** name)
{
  ...
  if ( i > 1 )
    {
    *name = (char*) allocateb( i*sizeof(char) );
    strcpy(*name,n);
    }
  else
    {
...
void SetNumberOfAllocatedPartitions(PARTITIONING* P, int newsize)
{
  ...
  if (P->AllocatedSize < newsize)
    ...
    P->First   = (int*) allocateb(newsize * sizeof(int));
    P->Uniques = (int*) allocateb(newsize * sizeof(int));
    P->CC      = (COUNTER*)allocateb(newsize * sizeof(COUNTER));
    ...
    for(i = P->PartitionCount; i < newsize; i++)
      {
      FirstVector(P, i) = ENDPARTITION;
      P->CC[i].counter  = (llong*)allocateb(P->Vsize * sizeof(llong));
      for( k = 0; k < P->Vsize; k++ )
        {
        CCScalar(P, i, k) = 0;
        }
    ...
    deallocateb(oldfirst);
    deallocateb(oldunique);
    deallocateb(oldCC);
    }
}
...
void FillEmptyPartitions(TRAININGSET* TS, CODEBOOK* CB, PARTITIONING* P)
{
  ...
  NewIndex  = (int*)allocateb((nEmpties + 1) * sizeof(int));
  error     = (llong*)allocateb((nEmpties + 1) * sizeof(llong));
                           ...
                           int           PartitionCount)
{
  ...
  P->Map            =  (int*)allocateb(BookSize(TS) * sizeof(int));
  P->First          =  (int*)allocateb(PartitionCount * sizeof(int));
  P->Next           =  (int*)allocateb(BookSize(TS) * sizeof(int));
  P->Uniques        =  (int*)allocateb(PartitionCount * sizeof(int));
  P->Vsize          =  VectorSize(TS);
  P->AllocatedSize  =  PartitionCount;

  P->CC = (COUNTER*)allocateb(P->PartitionCount * sizeof(COUNTER));
  for( i = 0; i < P->PartitionCount; i++ )
    {
    P->CC[i].counter = (llong*)allocateb(P->Vsize * sizeof(llong));
    }
...
void FreePartitioning(PARTITIONING* P)
{
  ...
  deallocateb(P->First);
  deallocateb(P->Next);
  deallocateb(P->Uniques);
  for( i = 0; i < P->AllocatedSize; i++ )
    {
    deallocateb(P->CC[i].counter);
    }
  deallocateb(P->CC);
}
...
static int ReadPartitioningFileHeader(FILE* f,
                                       PARTITIONING* P,
                                       TRAININGSET* TS)
/* allocates also the memory needed */
{
  ...
  if (fgets(P->Versionstr, MaxVersionLength, f) != NULL)
  ...
  if (tempchar != '-')
    {
    if (fgets(P->GenerationMethod, MaxGenMethodLength, f) != NULL)
                                ...
                                DISTANCETYPE  disttype)
{
  ...
  while (!EndOfPartition(i))
    {
    inew = FindNearestVector(&Node(TS,i), CB, &error, Map(P,i),disttype);
    j   = i;
    i   = NextVector(P,i);

    if (inew != clusterindex)
        ChangePartition(TS, P, inew, j);
    }
}
                                          ...
                                          DISTANCETYPE  disttype)
{
  ...
  llong     totalerror = 0;
  int       i;
  YESNO     usefreqs = (YESNO)( RecognizeCBFileType(TS) != RecognizeCBFileType(CB) );
...
static CBFILETYPE AnalyzeFileType(char* FileName)
{
  ...
  f = FileOpen(FileName, INPUT, NO);
  if (fgets(s, MaxVersionLength, f) == NULL)
	return NOTFOUND;
  fclose(f);
  ...
  while ( orderstring[i] )
    ...
    switch (orderstring[i] - '0')
      ...
      case 1:
        ...
        strcpy(s, FileName);
        CheckFileName(s, (char *)FormatNameTS);
        filetype = AnalyzeFileType(s);
        if(filetype!=NOTFOUND)
          {
          strcpy(FileName, s);
      ...
      case 2:
        ...
        strcpy(s, FileName);
        CheckFileName(s, (char *)FormatNameCB);
        filetype = AnalyzeFileType(s);
        if(filetype!=NOTFOUND)
          {
          strcpy(FileName, s);
      ...
      case 3:
        ...
        strcpy(s, FileName);
        CheckFileName(s, (char *)FormatNamePA);
        filetype = AnalyzeFileType(s);
        if(filetype!=NOTFOUND)
          {
          strcpy(FileName, s);
...
int GraphPutVector(Graph *g, int i, int *data)
{
  ...
  v = (GraphVector *) allocateb(sizeof(GraphVector));
  if (v == NULL)
    return 1;
  v->data = (int *) allocateb(sizeof(int)*g->dim);
  if (v->data == NULL)
    return 1;
  v->kindices = (int *) allocateb(sizeof(int)*g->k);
  if (v->kindices == NULL)
    return 1;

  v->data = (int *)memcpy((int *)v->data, (int *)data, sizeof(int)*g->dim);
...
{
  int nvec, k, dim, i, ret;
  Graph *g;

  ret = fscanf(f, "Graph v. %i", &i);  // Read version
  ret = fscanf(f, "%i\n", &nvec);
  ret = fscanf(f, "%i\n", &k);
  ret = fscanf(f, "%i\n", &dim);
...
Graph *AllocateMemoryForGraph(int number_vectors, int k, int dim) 
{
  ...
  g = (Graph *) allocateb(sizeof(Graph));
  if (g == NULL)
    return NULL;
  ...
  g->vectors = (GraphVector **) allocateb(sizeof(GraphVector) * 
                       GraphGetNumberVectors(g));
  if (g->vectors == NULL) {
    printf("Allocate error: AllocateMemoryForGraph(Graph)\n");
    return NULL;
  }
...
GraphVector *AllocateMemoryForGraphVector(Graph *g, int index) 
{
  ...
  v = (GraphVector *) allocateb(sizeof(GraphVector));
  v->data = (int *) allocateb(sizeof(int)*GraphGetDim(g));
  v->kindices = (int *) allocateb(sizeof(int)*GraphGetK(g));
  v->index = index;

  if (v == NULL || v->data == NULL || v->kindices == NULL) {
    printf("Allocate error: AllocateMemoryForGraphVector()\n");
    return NULL;
  }
...
GraphVector *ReadGraphVectorFromFile(FILE *f, Graph *g, int index)
{
  ...
  buffer = (char *) allocateb(sizeof(char)*LINEBUF);
...
void GraphWrite(char* FileName, Graph *g,  int AllowOverWrite)
{
  ...
  for(i=0; i<g->nvec; i++) {
    //    printf("Pointterit: %p %p\n", GraphGetVector(g, i), g->vectors[i]);
