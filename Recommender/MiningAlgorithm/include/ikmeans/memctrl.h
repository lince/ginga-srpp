...
/* Uncomment the following #define if you want to exit when out of
   memory occurs. Otherwise 'allocateb' and 'reallocateb' return NULL in
   the case of failure.
...
/* Do NOT use these five functions directly!
   Instead, use macros allocateb, deallocateb etc.
*/
...
#define allocateb(nbytes)          allocatememory(nbytes,__FILE__,__LINE__)
#define deallocateb(block)         deallocatememory(block,__FILE__,__LINE__)
#define reallocateb(block,nbytes)  reallocatememory(block,nbytes,__FILE__,__LINE__)
...
#define allocateb(nbytes)          malloc(nbytes)
#define deallocateb(block)         free(block)
#define reallocateb(block,nbytes)  realloc(block,nbytes)
